...
make_groups<-function(vista18TripsCsv, 
                      filterCsv, 
  ...
  #vista18TripsCsv <- '../data/VISTA_12_18_CSV.zip.dir/P_VISTA1218_V1.csv'
  ...
  #out_weekday_groups_csv_prefix <- '../output/1.setup/vista_2012_18_extracted_group_weekday_'
  
  extract_persons <- function(vista18TripsCsv, out_weekday_persons_csv_gz, out_weekend_persons_csv_gz) {
  ...
  extract_groups <- function(weekday_persons_csv_gz, filterCsv, out_weekday_groups_csv_prefix) {
    ...
    for (gid in groups) {
      outfile <- paste0(out_weekday_groups_csv_prefix,gid,".csv")
    ...
    for (row in 1:nrow(filters)) {
      ...
      outfile <- paste0(out_weekday_groups_csv_prefix,filters[row,]$cluster_id_5,".csv")
  ...
  extract_persons(vista18TripsCsv, out_weekday_persons_csv_gz, out_weekend_persons_csv_gz)
  echo(paste0('Extracting VISTA groups based on ',filterCsv,'\n'))
  extract_groups(out_weekday_persons_csv_gz, filterCsv, out_weekday_groups_csv_prefix)
