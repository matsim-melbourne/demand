...
vista_data_weekday <- vista_data %>%
  ...
  dplyr::select("PERSID","DESTPURP1","WDTRIPWGT") %>%
...
vista_data_cohorts <- vista_data_weekday %>%
  ...
  dplyr::select(age_group,sex,Activity,percentage) %>%
  ungroup() %>%
...
cohorts <- vista_data_cohorts %>%
  dplyr::select('Work','Study','Shop','Personal','Social/Recreational')%>%
  as.matrix()
rownames(cohorts)<-vista_data_cohorts$id
...
fviz_cluster(list(data = cohorts, cluster = sub_grp))
