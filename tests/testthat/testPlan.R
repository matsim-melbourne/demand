...
test_that("VISTA-like plans generation works", {
  ...
  wd<-getwd()
  setwd('../../R')
  csv<-paste0('../tests/expected/1.setup/vista_2012_18_extracted_activities_weekday_time_bins.csv.gz')
  endcsv<-paste0('../tests/expected/1.setup/vista_2012_18_extracted_activities_weekday_end_dist_for_start_bins.csv.gz')
  binCols<-3:50 # specifies that columns 3-50 correspond to 48 time bins, i.e., 30-mins each
  outdir<-'../tests/actual/3.plan'
